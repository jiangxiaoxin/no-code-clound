...
- 实现或明显改动表单组件（尤其是容器、联动、选择数据、流水号、成员/部门等复杂控件）后，**同一轮工作里**更新测试用例，不要只写规格。
- 至少三处：该能力的专项文件（没有就新建）、组合用例（当前为 `docs/testcases/2026-08-31-complex-form-manual-tests.md`）、纲领 `docs/testcases/README.md`（目录、执行顺序、准备、破坏性步骤、不要测）。
- 联合测试必须考虑与已有复杂能力的互动：标签页、子表单、数据联动 / 选择数据、流水号。包括能否放进标签页或子表、未打开 pane 是否仍校验/联动、flatten 是否多列、校验是否跳 pane、删除父容器后的内容。
