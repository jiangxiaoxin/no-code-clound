-- Flowable 7.1.0 官方 MySQL 建表脚本存档（流程引擎所需四段）
-- 来源：
--   common   ← flowable-engine-common-7.1.0.jar!/org/flowable/common/db/create/flowable.mysql.create.common.sql
--   engine   ← flowable-engine-7.1.0.jar!/org/flowable/db/create/flowable.mysql.create.engine.sql
--   history  ← flowable-engine-7.1.0.jar!/org/flowable/db/create/flowable.mysql.create.history.sql
--   identity ← flowable-idm-engine-7.1.0.jar!/org/flowable/idm/db/create/flowable.mysql.create.identity.sql
-- 用法：先建库（2026-09-09-flowable-demo-create-database.sql），再执行本文件（common 段必须在最前，
--       engine 段的外键/索引引用它建的表）；或不执行本文件，由示例首次运行自动建表（databaseSchemaUpdate=true）。
-- 覆盖范围：流程引擎（process）运行所需的全部表。Spring Boot 版还会用到 CMMN / DMN / 事件注册等
--          其它引擎的表，这些未入档，由引擎按 databaseSchemaUpdate=true 自动补建。
USE flowable_demo;

-- ============ 1/4 公共表（ACT_GE_* / ACT_RU_ 任务·变量·作业·批次等；官方 common 段还含 ACT_HI_ENTITYLINK 等几张公共历史表） ============
...
-- ============ 2/4 流程定义与执行（ACT_RE_* / ACT_RU_EXECUTION / ACT_EVT_LOG 等） ============
...
-- ============ 3/4 历史表（ACT_HI_*；其余几张公共历史表在 1/4 公共段） ============
