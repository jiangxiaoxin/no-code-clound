...
日期：2026-08-22  
状态：已实现（管理后台组织与角色）  
范围：管理后台入口、人员管理、部门管理、角色管理，以及支撑这些功能的单部门、多角色权限模型。
...
- 租户、公司或组织空间隔离。
- 岗位、汇报关系、兼任比例、主部门。部门负责人见 `docs/superpowers/specs/2026-09-07-dept-leader-design.md`。
- 按部门限定数据可见范围。
- 应用级角色和应用级权限。
- 审批流、操作审计日志、批量导入导出。
- 用户自行注册后等待审批。
...
用户与部门通过 `user_department` 关联，`userId` 唯一，因此一个人最多属于一个部门。关联只表达成员归属，不从部门继承角色。不设主部门或兼任。
...
- `POST /api/admin/users`：创建账号并一次性分配一个部门和多个角色。`departmentId` 可空。
...
- 新增和编辑使用抽屉或对话框，部门用单选的树选择（一个人员只属一个部门），角色用多选下拉。
- 操作包括编辑、启停、重置密码。
- 按权限隐藏或禁用按钮。
